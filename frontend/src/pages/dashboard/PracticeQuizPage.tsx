...
export function PracticeQuizPage() {
  ...
  const diffColor = (d: string) => d === 'easy' ? 'text-primary bg-primary/10' : d === 'hard' ? 'text-red-500 bg-red-500/10' : 'text-amber-500 bg-amber-500/10';
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-6">
              <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                <div className="h-full bg-primary transition-all" style={{ width: `${((currentIdx + (result ? 1 : 0)) / questions.length) * 100}%` }} />
              </div>
            ...
            <motion.div key={currentIdx} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-card rounded-2xl border p-6">
                ...
                {q.options.map((opt, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      {result && opt === result.correctAnswer && <CheckCircle className="w-5 h-5 text-primary ml-auto" />}
              ...
              {result && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`p-4 rounded-xl mb-4 ${result.isCorrect ? 'bg-primary/10 border border-primary/20' : 'bg-red-500/10 border border-red-500/20'}`}>
